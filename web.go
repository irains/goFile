...
func themePrepaint(value string) string {
	...
	return `<script nonce="` + nonce + `">(function(){var mode="system";var resolved="light";try{var raw=localStorage.getItem("fileharbor-mode");if(raw==="light"||raw==="dark"||raw==="system"){mode=raw}}catch(_){ }if(mode==="light"){resolved="light"}else if(mode==="dark"){resolved="dark"}else{resolved=(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)?"dark":"light"}var root=document.documentElement;root.setAttribute("data-mui-color-scheme",resolved);}());</script><style nonce="` + nonce + `">:root{color-scheme:dark;background-color:#171D18;background-image:radial-gradient(circle at 1px 1px,rgb(219 231 207 / 2.6%) 0 .65px,transparent .8px),linear-gradient(118deg,rgb(128 159 113 / 2%),transparent 42%,rgb(100 172 180 / 1.8%));background-size:13px 13px,100% 100%;color:#F0F1E7}:root[data-mui-color-scheme="light"]{color-scheme:light;background-color:#F3F0E7;background-image:radial-gradient(circle at 1px 1px,rgb(67 84 54 / 4%) 0 .65px,transparent .8px),linear-gradient(118deg,rgb(89 111 74 / 2.5%),transparent 42%,rgb(37 99 109 / 2.5%));background-size:13px 13px,100% 100%;color:#242820}</style>`
}
