...
func useTestRoot(t *testing.T) string {
	t.Helper()
	previous := conf.GoFile
	root := t.TempDir()
	conf.GoFile = root
	t.Cleanup(func() { conf.GoFile = previous })
	return root
}
...
func TestFileOperationsRejectConflictsAndSelfDescendants(t *testing.T) {
	root := useTestRoot(t)
	if err := os.Mkdir(filepath.Join(root, "source"), 0755); err != nil { t.Fatal(err) }
	if err := os.WriteFile(filepath.Join(root, "source", "a.txt"), []byte("a"), 0644); err != nil { t.Fatal(err) }
	if err := os.Mkdir(filepath.Join(root, "target"), 0755); err != nil { t.Fatal(err) }
	if _, err := CopyItem("source/a.txt", "target", ""); err != nil { t.Fatal(err) }
...
func TestBatchCopyPreservesPreexistingStagingPath(t *testing.T) {
	root := useTestRoot(t)
	if err := os.WriteFile(filepath.Join(root, "a.txt"), []byte("a"), 0644); err != nil { t.Fatal(err) }
	if err := os.WriteFile(filepath.Join(root, "b.txt"), []byte("b"), 0644); err != nil { t.Fatal(err) }
	if err := os.Mkdir(filepath.Join(root, "target"), 0755); err != nil { t.Fatal(err) }
	staging := filepath.Join(root, "target", "b.txt.gofile-copy-staging")
	if err := os.WriteFile(staging, []byte("keep"), 0644); err != nil { t.Fatal(err) }
	list, err := ListDirectory("")
	if err != nil { t.Fatal(err) }
	allowed := make(map[string]string)
	for _, entry := range list.Entries { if entry.Name == "a.txt" || entry.Name == "b.txt" { allowed[entry.Name] = entry.Version } }
	selection, err := ValidateSelection("", allowed, []ItemRequest{{Name:"a.txt", Version:allowed["a.txt"]}, {Name:"b.txt", Version:allowed["b.txt"]}})
	if err != nil { t.Fatal(err) }
	if _, err := BatchCopy(selection, "target"); ErrorCode(err) != "destination_busy" { t.Fatalf("expected staging conflict, got %v", err) }
	data, err := os.ReadFile(staging)
	if err != nil || string(data) != "keep" { t.Fatalf("pre-existing staging path was changed: %q, %v", data, err) }
	if _, err := os.Lstat(filepath.Join(root, "target", "a.txt.gofile-copy-staging")); !errors.Is(err, os.ErrNotExist) { t.Fatalf("request staging path remained: %v", err) }
}
...
func TestBatchDeleteReportsChangedItem(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if err := os.Remove(aAbsolute); err != nil { t.Fatal(err) }
	...
	if len(results) != 2 { t.Fatalf("expected two results, got %#v", results) }
	codes := map[string]string{}
	for _, result := range results { codes[result.Name] = result.Code }
	if codes["a.txt"] != "not_found" || codes["b.txt"] != "deleted" { t.Fatalf("unexpected deletion results: %#v", results) }
}
...
func TestBatchDeleteRejectsMismatchedSelectionDirectory(t *testing.T) {
	...
	if len(results) != 1 || results[0].Code != "source_changed" { t.Fatalf("expected source_changed result, got %#v", results) }
	if _, err := os.Lstat(absolute); err != nil { t.Fatalf("mismatched selection deleted the source: %v", err) }
}
...
func TestBatchOperationRejectsMismatchedSelectionDirectory(t *testing.T) {
	...
	if _, err := BatchCopy(selection, "target"); ErrorCode(err) != "source_changed" { t.Fatalf("expected source_changed rejection, got %v", err) }
	if _, err := os.Lstat(absolute); err != nil { t.Fatalf("mismatched selection copied or removed the source: %v", err) }
}
...
func TestBatchMoveUsesSingleVolume(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	var version string
	for _, entry := range list.Entries { if entry.Name == "a.txt" { version = entry.Version } }
	selection, err := ValidateSelection("", map[string]string{"a.txt": version}, []ItemRequest{{Name:"a.txt", Version:version}})
	if err != nil { t.Fatal(err) }
	if err := ensureSameVolume(selection, "target"); err != nil { t.Fatalf("same-volume destination rejected: %v", err) }
}
